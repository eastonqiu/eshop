...
class ControllerPaymentWxPay extends Controller {
	...
	public function index() {
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('payment/wxpay', 'token=' . $this->session->data['token'], true)
		);

		$data['action'] = $this->url->link('payment/wxpay', 'token=' . $this->session->data['token'], true);
		...
		$data['entry_wxpay_token'] = $this->language->get('entry_wxpay_token');
		...
		if (isset($this->request->post['wxpay_token'])) {
			$data['wxpay_token'] = $this->request->post['wxpay_token'];
		} else {
			$data['wxpay_token'] = $this->config->get('wxpay_token');
		}		
	...
	public function install() {
        $this->db->query( "CREATE TABLE  IF NOT EXISTS `" . DB_PREFIX . "weixin_account` (
    ...
    public function uninstall() {
        $this->db->query("DROP TABLE IF EXISTS `" . DB_PREFIX . "weixin_account`");
    }
}